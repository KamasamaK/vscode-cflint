...
export const CFLINT_DIAGNOSTIC_SOURCE: string = "cflint";
...
function makeDiagnostic(document: TextDocument, issue: CFLintIssue): Diagnostic {
  ...
  const diagnosticRange: Range = wordRange ? wordRange : new Range(start, start);
  let diagnosticTags: DiagnosticTag[] = [];
